...
const PictureCard = () => {
 ...
 //Creates the Menu above the gallery
  function createPictureMenu(text) {
    ...
    return (
      <div onClick={handleClick} className={activeMenu==text?"bg-[#2ae8d1] text-white rounded hover:duration-300 hover:ease-in-out hover:cursor-pointer hover:bg-stone-700  text-white px-4 py-2":"hover:duration-300 hover:ease-in-out hover:cursor-pointer hover:bg-[#6C6C6C] hover:text-white hover:rounded text-[white] px-4 py-2"}  key={text}>
        <p>{text}</p>
      </div>
    )
  }
